    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _ContactsCard(contactsAsync: contactsAsync, simpleMode: simpleMode, theme: theme),
        ],
      ),
    );
  }
...
class _StatusBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = !hasLocation ? theme.colorScheme.onSurfaceVariant : theme.colorScheme.tertiary;
    ...
    final tint = hasLocation
        ? Color.alphaBlend(color.withValues(alpha: 0.12), theme.scaffoldBackgroundColor)
        : null;
    return Card(
      color: tint,
      child: Padding(
          ...
          children: [
            ...
            Flexible(
              child: Column(
                ...
                children: [
                  Text(
                    hasLocation ? 'Zona sin alertas cercanas' : 'Obteniendo ubicación…',
              ...
              simpleMode
                  ? Container(
                      ...
                      child: const Row(children: [
                        Text('✓ Ubicación activa', style: TextStyle(fontWeight: FontWeight.w600)),
                      ]),
                    )
                  : Text(
...
class _FrequentPlacesCard extends ConsumerWidget {
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Icon(Icons.navigation_outlined, color: Theme.of(context).colorScheme.primary),
              const SizedBox(width: 8),
              const Text('Lugares frecuentes', style: TextStyle(fontWeight: FontWeight.w600)),
              const Spacer(),
              Text('${places.length} guardados',
              ...
              Wrap(
                ...
                children: places
                    .map((p) => _PlaceChip(
                          place: p,
                          onDelete: () =>
                              ref.read(frequentPlacesProvider.notifier).remove(p.id),
                        ))
                    .toList(),
              ),
  ...
  Future<void> _showAddPlaceDialog(
      BuildContext context, WidgetRef ref, LocationState location) async {
                ...
                children: [
                  ...
                  TextField(
                    ...
                    onChanged: (value) {
                      ...
                      debounce = Timer(const Duration(milliseconds: 500), () async {
                        ...
                        if (context.mounted) setState(() => suggestions = results);
                      });
                    },
                  ),
                  ...
                  if (geocodedLat != null)
                    const Padding(
                      ...
                      child: Text('✓ Dirección geocodificada correctamente',
...
class _PlaceSuggestion {
  ...
  final double lat;
  final double lon;
  _PlaceSuggestion({required this.displayName, required this.lat, required this.lon});
}
...
class _ContactsCard extends ConsumerWidget {
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              ...
              const SizedBox(width: 8),
              const Text('Contactos de emergencia', style: TextStyle(fontWeight: FontWeight.w600)),
            ]),
            const SizedBox(height: 12),
            contactsAsync.when(
              ...
              data: (contacts) => Column(
                children: [
                  if (contacts.isEmpty)
                    const Padding(
                      padding: EdgeInsets.symmetric(vertical: 12),
                      child: Center(child: Text('Sin contactos agregados todavía.')),
                    )
                  else
                    ...contacts.asMap().entries.map((entry) => _ContactTile(
                          index: entry.key + 1,
                          contact: entry.value,
                          simpleMode: simpleMode,
                          onEdit: () => _showContactDialog(context, ref, editing: entry.value),
                        )),
                  const SizedBox(height: 8),
                  OutlinedButton.icon(
  ...
  Future<void> _showContactDialog(BuildContext context, WidgetRef ref,
      {EmergencyContact? editing}) async {
    ...
    String? relationship =
        _relationshipLabels.containsKey(editing?.relationship) ? editing?.relationship : null;
        ...
        builder: (context, setState) => AlertDialog(
                      ...
                      return Expanded(
                        child: Padding(
                          ...
                          child: OutlinedButton(
                            onPressed: () => setState(() => importance = e.key),
          ...
          actions: [
            TextButton(
                onPressed: () => Navigator.pop(context), child: const Text('Cancelar')),
            FilledButton(
              onPressed: (nameController.text.isEmpty || phoneController.text.isEmpty)
                  ? null
                  : () async {
      ...
      title: Row(children: [
        ...
        if (!simpleMode) ...[
          ...
          Container(
            ...
            child: Text(_importanceLabels[contact.importance] ?? '',
